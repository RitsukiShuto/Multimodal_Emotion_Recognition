...
def main():
    # 出力先
    full_labeled_dir = "../vector/pow_spectrum/full_labeled_pow.csv"
    half_labeled_dir = "../vector/pow_spectrum/half_labeled_pow.csv"
    un_labeled_dir = "../vector/pow_spectrum/un_labeled_pow.csv"
    ...
    for dir in dir_list:                        # wavファイルのあるディレクトリを1つずつ走査
        ...
        for wav in wav_list:                    # wavファイルを1つずつ走査
            ...
            elif dir == "../data/wav/half_labeled":     # 'ans_n'ラベル付き
    ...
    # CSVで保存
    np.savetxt(full_labeled_dir, full_labeled_pow, fmt='%8f', delimiter=',')
    np.savetxt(half_labeled_dir, half_labeled_pow, fmt='%8f', delimiter=',')
