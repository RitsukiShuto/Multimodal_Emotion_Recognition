...
def make_ogvc2_metadata(wav_list):
    ...
    for row in ref_text.values:
        if row[2] in ['ANG', 'JOY', 'NEU', 'SAD', 'SUR']:
            for lv in range(4):
...
def main():
    # 処理するwaxファイルのリストを取得
    wav_dir = "data/wav/OGVC_vol2/all_data/"       # wavファイルのディレクトリ
    wav_list_F = glob.glob(wav_dir+"F*.wav")        # 女性の演技音声のリストを取得
    wav_list_M = glob.glob(wav_dir+"M*.wav")        # 男性の演技音声のリストを取得

    df = make_ogvc2_metadata(wav_list_F)
    df.to_csv('data/OGVC_mixed_F.csv', index=True, header=1)

    df = make_ogvc2_metadata(wav_list_M)
    df.to_csv('data/OGVC_mixed_M.csv', index=True, header=1)
