...
def calc_conf_mat(conf_mats, experiment_times, save_dir):
    ...
    df1.to_csv(save_dir+'/prob_conf_mat.csv')
    df2.to_csv(save_dir+'/num_conf_mat.csv')

def save_fig(save_dir, multimodal_model, history_MM, num, cnt):
    ...
    # 学習ログ(.csv)
    df1 = pd.DataFrame(history_MM.history)      # DataFrame化
    df1.to_csv(save_dir + "/" + str(num) + '/train_log_MM' + str(f_num) + '.csv')     # csvで保存
